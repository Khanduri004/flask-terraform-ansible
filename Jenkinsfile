pipeline{
  ...
  environment {
    ...
    KEY_NAME = 'DOCKER'
    PRIVATE_KEY = credentials('ubuntu04')
}
  ...
  stages{
   ...
   stage('Clone Repo') {
     steps {
       https://github.com/Khanduri004/terraform-ansible-aws-project.git
    }
  }
   ...
   stage('Configure Inventory') {
            steps {
                script {
                    def publicIp = sh(returnStdout: true, script: "terraform -chdir=terraform output -raw public_ip").trim()
                    writeFile file: 'ansible/inventory.ini', text: "[appservers]\n${publicIp} ansible_user=ec2-user ansible_ssh_private_key_file=~/.ssh/id_rsa"
                }
            }
        }
